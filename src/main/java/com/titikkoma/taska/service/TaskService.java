...
public class TaskService {
    ...
    public Task updateTaskStatus(String taskId, String newStatus) {
        ...
        int affected = this.taskRepository.updateStatusById(taskId, newStatus);
        if (affected == 0) {
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "Failed to update status");
        }

        task.setStatus(newStatus); // Update status in memory

        Map<String, Object> content = new HashMap<>();
        content.put("updated_by_id", principal.getId());
        content.put("updated_by_name", principal.getName());
        content.put("old_status", oldStatus);
        content.put("new_status", newStatus);
    ...
    public List<Log> getLogsByTaskId(String taskId) {
        Map<String, Object> conditions = new HashMap<>();
        conditions.put("reference_id", taskId);
        conditions.put("type", "task");

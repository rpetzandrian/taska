...
public class OrganizationController {
    ...
    @PutMapping("/{id}")
    public WebResponse<Organization> update(
            @PathVariable String id,
            @RequestBody Organization organization
    ) {
        Organization updated = service.updateOrganization(id, organization);
        return WebResponse.<Organization>builder().data(updated).build();
    }

    @DeleteMapping("/{id}")
    public WebResponse<String> delete(@PathVariable String id) {
        service.deleteOrganization(id);
        return WebResponse.<String>builder().data("organization_deleted").build();
    }
}
